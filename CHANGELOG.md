...
- **Coverage recovery + TS-eslint 8** — Codecov `ignore:` aligned with Vitest showcase excludes; backend `internal/wire/` tests; frontend branch coverage raised (Vitest floors **75/54/78/75**). `@typescript-eslint/*@8.60`, `eslint-plugin-solid@0.14.5` on ESLint 8 (clears minimatch audit chain). Remaining npm audit high: `h3` via `vinxi`/`@solidjs/start` — accepted dev/SSR build risk; do not `npm audit fix --force`
- **Test counts** — 784 total (257 Go unit + 507 Vitest + 20 Playwright E2E)
