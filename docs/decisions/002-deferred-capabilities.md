...
After completing the V1 starter (auth, 70+ components, CI/CD, 993 tests; Codecov ~78% project and recovering toward 80%+ per [coverage-and-eslint plan](../plans/coverage-and-eslint.md), deployment infra), we evaluated what additional engineering capabilities a "rebuild Notion at Google level" project would need. Twenty-one capabilities were identified as gaps between "production starter" and "scaled production service," or flagged during the pre-launch audit as improvements that aren't worth the cost right now.
...
- **Testing:** 993 tests (351 Go + 622 Vitest + 20 Playwright E2E); Codecov ~78% project (recovery toward 80%+; Vitest floors 75/54/78/75 on included files), CI-enforced thresholds
