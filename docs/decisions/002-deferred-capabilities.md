...
After completing the V1 starter (auth, 70+ components, CI/CD, 784 tests; Codecov ~68% project (0.3 baseline), deployment infra), we evaluated what additional engineering capabilities a "rebuild Notion at Google level" project would need. Twenty-one capabilities were identified as gaps between "production starter" and "scaled production service," or flagged during the pre-launch audit as improvements that aren't worth the cost right now.
...
- **Testing:** 784 tests; Codecov ~68% project (0.3 baseline) (unit + integration + component + E2E), CI-enforced thresholds
