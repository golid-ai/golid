...
func TestSetupDomainDefaultsRenamed(t *testing.T) {
	t.Parallel()

	dir := t.TempDir()
	path := filepath.Join(dir, "scripts/setup-domain.sh")
	content := `GCP_PROJECT="${GCP_PROJECT:-golid-app}"
...
`
	...
	if replaceInFileSafe(path, oldProjectName, "myapp") != 1 {
		t.Fatal("expected project name replacement")
	}
	if replaceInFile(path, oldDomain, "myapp.co") != 1 {
		t.Fatal("expected domain replacement")
	}
	...
	body := string(got)
	if strings.Contains(body, "golid") {
		t.Fatalf("golid survivor in setup-domain.sh: %q", body)
	}
}
